...
CREATE INDEX IF NOT EXISTS tx_user_date_type_idx 
  ON public.transactions(user_id, occurred_on DESC, type)
  WHERE deleted_at IS NULL;

-- Index 2: Transactions filtering by type and date range
-- Useful for filtering income/expense by date range in API
CREATE INDEX IF NOT EXISTS tx_user_type_date_idx 
...
CREATE INDEX IF NOT EXISTS budgets_user_category_date_idx
  ON public.budgets(user_id, category_id, start_date, end_date)
  WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS accounts_user_balance_idx
  ON public.accounts(user_id, current_balance DESC)
  WHERE deleted_at IS NULL;
...
-- Index 8: Profile timezone queries
...
-- Index 9: Covering index for transaction lists
-- Includes commonly queried columns to avoid table lookups
CREATE INDEX IF NOT EXISTS tx_user_date_covering_idx
  ON public.transactions(user_id, occurred_on DESC, type, amount, account_id, category_id)
  WHERE deleted_at IS NULL;

-- Index 10: Category lookups by user and side (income vs expense)
-- Useful for filtering expense/income categories in forms
CREATE INDEX IF NOT EXISTS categories_user_side_idx
  ON public.categories(user_id, side, sort_order)
  WHERE deleted_at IS NULL;

-- Index 11: Audit log global time queries (for admin/reporting)
-- Useful for audit reports across all users
CREATE INDEX IF NOT EXISTS audit_occurred_at_idx
  ON public.audit_log(occurred_at DESC, table_name);
